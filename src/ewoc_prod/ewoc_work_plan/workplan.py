...
from .reproc import reproc_wp
from .s2prods import run_multiple_cross_provider
from .utils import (
    eodag_prods,
    get_path_row,
    greatest_timedelta,
...
class WorkPlan:
    def __init__(
        ...
        if s1_data_provider not in ["creodias", "astraea_eod"]:
            raise ValueError(f"Incorrect s1 data provider: {s1_data_provider}")
        if not set(s2_data_provider).issubset(
            ["creodias", "peps", "astraea_eod", "aws", "aws_sng"]
        ):
            raise ValueError(f"Incorrect s2 data provider: {s2_data_provider}")
        # Filling the plan
        self._plan = dict()
        #  Common MetaData
