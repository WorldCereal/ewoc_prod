...
You need to install libgdal-dev by running this command : ```sudo apt-get install libgdal-dev```

Install GDAL on your virtual environment by using the command ```pip install GDAL==$(gdal-config --version)```

Be sure to have the last version of eodag to avoid problems linked to the addition of new providers and the eodag config file is at the following place ```/.config/eodag/eodag.yml```
